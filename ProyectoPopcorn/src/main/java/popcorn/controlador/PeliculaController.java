...
public class PeliculaController {
    ...
    public String doEditarPelicula(@RequestParam("idPelicula") String idPelicula,
            ...
            @RequestParam("duracion") int duracion, @RequestParam("categoria") String categoria, @RequestParam("actores") String actores,
            @RequestParam("director") String director, @RequestParam("fechaEd") String fecha, @RequestParam("trailer") String trailer) {
        //System.out.println("AKI peliControler editarPeli 1 fechaED: " + fecha);
        final Pelicula pelicula = new Pelicula(titulo, sinopsis, duracion, categoria, director, fecha, trailer);
        final List<String> actor = new ArrayList<String>();
        StringTokenizer tokens = new StringTokenizer(actores, ",");
        while (tokens.hasMoreTokens()) {
    ...
    @RequestMapping(value = "/crear_categoria", method = RequestMethod.GET)
    public @ResponseBody String doCrearCategoria(@RequestParam("categoriaNueva") String categoria) {
        Categoria cat = new Categoria();
        cat.setNombre(categoria);
        categoriaService.create(cat);
        return datosCategoria().toString();
    }
    
    private JSONObject datosCategoria() {
        JSONObject jsonObject = new JSONObject();
        try {
            jsonObject.put("categoriasNuevas", categoriaService.getAllCategorias());
        } catch (JSONException ex) {
            Logger.getLogger(PeliculaController.class.getName());
        }
        return jsonObject;
    }
}